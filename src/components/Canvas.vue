...
import axios from 'axios'
    ...
    export default {
        ...
        fill(e){
            ...
            this.layer.batchDraw();
            axios.post( this.url +'paint/update?id='+ e.target.id() ,this.createPost(e.target))
        },
        ...
        erase(e){
            if(e.target != this.stage){
                axios.post( url +'paint/delete?id='+ e.target.id() ,this.createPost(e.target))
        ...
        copy(e,ind){
            if(e.target != this.stage){
                ...
                this.layer.add(copy)
                this.layer.batchDraw()
                axios.post( this.url +'paint/create' ,this.createPost(e.target))
           }
        },
        ...
        mounted() {
            this.emitter.on("tool-clicked",(data) =>{
                if(this.shapes.length != 1){
                    axios.post( this.url +'paint/create',this.createPost(this.shapes[this.shapes.length - 1]))
            ...
            this.stage.on('click',(e)=>{
                //=========> read this then remove it : kont 3amelha hasName('shape') bs la2et eny m7tag asmy kol wa7ed menhom
